...
### Steps
**1.** `Added MiddleName` property to the `Student` entity, and removed the old migration.  
...
I then removed the old migration by using the command `dotnet ef migrations remove`.  
...
### Steps
**1.** `Added DateOfBirth` property to the `Student` entity, and removed the old migration.  
...
### Steps
**1.** Defined `Instructor` & updated `Course` entity, and removed the old migration.
